...
@Aspect
@Component
public class DeclareLoggingAspect {
	/*
	 * 1.定义一个方法，用于声明切入点表达式。 一般的，该方法中不需要再添加其它的代码
	 * 2.使用  @Pointcut 来声明切入点表达式
	 * 3.后面的其它的通知直接使用方法名来引用当前的切入点表达式
	 */
	@Pointcut("execution(public int com.test.spring.aop.aspect.ArithmeticCalculator.*(..))")
